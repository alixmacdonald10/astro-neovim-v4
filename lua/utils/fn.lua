...
function M.get_files_by_end(fileType, path)
  ...
  if #find > 1 then
    for _, value in ipairs(find) do
      ...
      -- Include directory in display for clarity
      local display_path = new_path:gsub("^/", "") -- Remove leading slash for better readability
      table.insert(files, { tostring(display_path), tostring(value) })
    end
  end
  return files
end
